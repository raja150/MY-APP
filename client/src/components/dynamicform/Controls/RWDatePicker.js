...
const RWDatePicker = (props) => {
    ...
    const handleChange = async (date) => {
        ...
        if ((editFormat == "MM/YY" || editFormat == "MMM/YY") && props.dateType) {
            ...
            handlevaluechange(props.name,
                showTime === 1 || showTime === true ?
                    Moment(date).format(Moment.HTML5_FMT.DATETIME_LOCAL_SECONDS) : Moment(date).format(Moment.HTML5_FMT.DATE)
                , {});
        }
    }
    if (format == undefined) {
    ...
    return (
        <FormGroup >
            ...
            <DateTimePicker
                ...
                className={cssClass}
                date={showDate === true ? true : false}
                time={showTime === true ? true : false}
