...
import { MORE_DETAILS } from '../../Approvals/navigation';
...
export default function LMApproveLeave(props) {
    const [state, setState] = useState({
        isLoading: true, formValues: {}, Employees: [], appliedLeaves: [], approvedLeaves: [],
         request: [], weekLeaves: []
    })
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            if (rid) {
                await ApplyLeavesService.getLeaveById(rid).then((result) => {
                    request = result.data
                });
                ...
                setState({ ...state, isLoading: false, request: request, weekLeaves: weekLeaves, appliedLeaves: appliedLeaves, approvedLeaves: approvedLeaves })//, weekLeaves: weekLeaves
    ...
    const MyCalendar = async (e) => {
        ...
        history.push(MORE_DETAILS + queryString.stringify(qry));
    };
