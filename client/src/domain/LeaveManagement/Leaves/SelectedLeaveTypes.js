...
function SelectedLeaveTypes({ form, push, remove, ...props }) {
    ...
    const handleValueChange = async (name, value, selected, index) => {
        ...
        if (value != "") {
            ...
            await ApplyLeavesService.GetLeaveBalanceByEmpLeaveType(value, values.employeeId, values.fromDate, values.toDate).then((res) => {
                ...
                values.selectedLeaveTypes.map((leaveType, index) => {
                    ...
                    return (
                        <>
                            ...
                            }} handlevaluechange={(e, v, selected) => handleValueChange(e, v, selected.selected, index)} />
                        </>
                    )
                })}
