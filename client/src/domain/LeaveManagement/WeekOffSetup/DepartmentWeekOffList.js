...
export default function DepartmentWeekOffList(props) {
    const DeptColumns = [
        {
    ...
    const fetchData = async () => {
        ...
        await DepartmentService.getDeptWeekOffLists(qString).then((res) => {
    ...
    const handleDelete = async (n) => {
        await DepartmentService.DeleteWeekOffSetup(n).then((res) => {
    ...
    return <>
        <Card className='mb-4'>
            <Row>
                ...
                <Col md='2'>
                    <Button className="mb-2 mr-2 btn-icon float-right btn-xs btn-primary1" color="primary" type="button"
                        onClick={() => props.handleAddDept(null)} 
                    >
            ...
            <Table columns={DeptColumns} data={state.weekOffs} pages={state.pages} hasPrevious={state.hasPrevious} hasNext={state.hasNext} pageIndex={state.pageIndex} showPaginate={true}  handleSearch={handleSearch} />
