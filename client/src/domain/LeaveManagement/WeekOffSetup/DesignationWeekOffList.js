...
export default function DesignationWeekOffList(props) {
    ...
    const qString = queryString.stringify({
     ...
     await DesignationService.getDesignWeekOffList(qString).then((res) => {
...
const handleSearch = async (page, pageSize, sortBy, isDescend) => {
    ...
    const handleDelete = async (n) => {
        await DesignationService.DeleteWeekOffSetup(n).then((res) => {
    ...
    return <>
        <Card className='mb-4'>
            <Row>
                ...
                <Col md='2'>
                    <Button className="mb-2 mr-2 btn-icon float-right btn-xs btn-primary1" color="primary" type="button"
                        onClick={() => props.handleAddDesign(null)}
                    >
            ...
            <Table columns={DesignColumns} data={state.weekOffs} pages={state.pages} hasPrevious={state.hasPrevious} hasNext={state.hasNext} pageIndex={state.pageIndex} showPaginate={true}  handleSearch={handleSearch} />
        </Card>
    </>
}