...
export default function LocationWeekOffList(props) {
    const LocColumns = [
        {
    ...
    const fetchData = async () => {
        ...
        await LocationService.getLocWeekOffList(qString).then((res) => {
    ...
    const handleDelete = async (n) => {
        await LocationService.DeleteWeekOffSetup(n).then((res) => {
            notifySaved('Deleted sucessfully');
            fetchData();
        }).catch((err) => {
    ...
    return <>
        <Card className='mb-4'>
            <Row>
                ...
                <Col md='2'>
                    <Button className="mb-2 mr-2 btn-icon float-right btn-xs btn-primary1" color="primary" type="button"
                        onClick={() => props.handleAddLoc(null)}
                    >
            ...
            <Table columns={LocColumns} data={state.weekOffs} pages={state.pages} hasPrevious={state.hasPrevious} hasNext={state.hasNext} pageIndex={state.pageIndex} showPaginate={true}  handleSearch={handleSearch} />
