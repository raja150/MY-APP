...
export default function DepartmentList(props) {
    ...
    const [state, setState] = useState({
        isLoading: true, depts: [], pages: -1, title: '',
    ...
    const SearchData = async (page, pageSize, sortBy, isDescend) => {
        ...
        await TestDepartmentService.Paginate(qString).then((res) => {
            pages = res.data.pages;
            hasNext = res.data.hasNext;
            hasPrevious = res.data.hasPrevious;
            pageIndex = res.data.index
            setState({
                ...state, isLoading: false, depts: res.data.items, pages: pages, hasNext: hasNext,
    ...
    const handleDelete = async (id) => {
        await TestDepartmentService.Delete(id).then((res) => {
    ...
    return <Fragment>
        <Card className='mb-4'>
            ...
            <Card>
                <CardBody>
                    <Table columns={DeptColumns} data={state.depts} pages={state.pages}
