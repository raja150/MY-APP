...
import { Input, Radio, RWReportDatePicker, RWDropdownList } from 'components/dynamicform/Controls';
...
export default function ReportSearchComponentNew(props) {
    const fieldMap = {
        Dropdown: RWDropdownList,
        DatePicker: RWReportDatePicker,
        Input: Input,
        Radio: Radio,
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            if (!_.isEmpty(queryProps)) {//&& queryProps.report == props.reportName
    ...
    const getTableData = async (queryValue) => {
        ...
        props.json.summaryTable && props.json.summaryTable.length > 0 &&
        ...
        //setStateNew({ reportsData: props.clearData ? [] : reportsData, summaryData: summaryTableData });
    ...
    const handleSearch = async () => {
        setIsSearching(true)
        // props.setClearData && props.setClearData(false)
            ...
            <Row>
                ...
                <Col xs='1'>
                    <Button className="mb-2 mr-2 mt-3 btn-icon btn-success1" key='buttonSave'
                        color='success' type="button" disabled={props.isSearching} onClick={() => handleSearch()} name="search"  >{props.isSearching ? 'Please wait' : 'Search'} </Button>
                </Col>
            </Row>
            {isSearching ?
                <Row>
