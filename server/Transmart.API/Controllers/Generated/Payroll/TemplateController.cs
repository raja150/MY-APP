...
namespace TranSmart.API.Controllers.Payroll
{
    ...
    public partial class TemplateController : BaseController
    {
        ...
        {
            return Ok(_mapper.Map<IEnumerable<TemplateList>>(await _service.GetList(OrderBy)));
        }
        ...
        {
            return Ok(_mapper.Map<Models.Paginate<TemplateList>>(await _service.GetPaginate(baseSearch)));
        }
        ...
        {
            return Ok(_mapper.Map<TemplateModel>(await _service.GetById(id)));
        }

        [HttpPost]
        [ApiAuthorize(Core.Permission.PS_Template, Core.Privilege.Create)]
        public async Task<IActionResult> Post(TemplateModel model)
        {
            Result<Template> result = await _service.AddAsync(_mapper.Map<Template>(model));
            if (result.IsSuccess)
            {
                return Ok(_mapper.Map<TemplateModel>(result.ReturnValue));
            }
        ...
        public async Task<IActionResult> Put(TemplateModel model)
        {
            Result<Template> result = await _service.UpdateAsync(_mapper.Map<Template>(model));
            if (result.IsSuccess)
            {
                return Ok(_mapper.Map<TemplateModel>(result.ReturnValue));
            }
