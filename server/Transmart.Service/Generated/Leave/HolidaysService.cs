...
namespace TranSmart.Service.Leave
{
    ...
    {
        public HolidaysService(IUnitOfWork uow) : base(uow)
        {

        }

        public override async Task<Holidays> GetById(Guid id)
        {
            return await UOW.GetRepositoryAsync<Holidays>().SingleAsync(
                predicate: x => x.ID == id,
                orderBy: o => o.OrderByDescending(x => x.AddedAt));
        }

        public override async Task<IPaginate<Holidays>> GetPaginate(BaseSearch search)
        {
            return await UOW.GetRepositoryAsync<Holidays>().GetPageListAsync(
